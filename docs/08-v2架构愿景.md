# 08. v2 多 Agent 架构愿景（2026-03 写就，持续指引至今）

> **本阶段在成长史中的位置**：这是项目早在 2026-03（v1 还在打磨时）就画下的「终局蓝图」——把 daemon 从「claude↔codex 单例直连」重新设想为 room-aware、身份明确、协议化的**纯消息路由器**。它描述的多 Agent 形态当时尚未实现，却像北极星一样指引后续每一步：阶段 06 的协作协议 v2、乃至阶段 09 今天的 v3 跨网协作系统规格，都在向这张蓝图收敛。把它放在最新规格（09）之前，是为了让你看清「早期愿景 → 最终落地」这条设计演进线。以下是 v2 架构设计原文（2026-03-22 与 Codex 讨论 + 二次评审整理；当时尚有英文版，现以中文版为准）。
...
> **与 v1 的关系**：以下部分能力已在 [v1 路线图](01-起步与v1协作核心.md) 中通过 prompt contract 提供了单桥轻量版。v2 的目标不是重做这些能力，而是将它们提升到多 agent、room-aware、协议级的正式基础设施。
...
- 详见 [v1 路线图 §5 v1.3](01-起步与v1协作核心.md)
