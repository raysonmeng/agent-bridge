...
2. **daemon 加第二个 bridge（per-agent-type）**（daemon.ts，共享文件，我先动）
   - 实现选择：**保留 `let roomBridge`（=Claude，Claude 工具处理器零改）+ 新增 `let codexRoomBridge`**，不引入 `Map<agentType,Handle>`。原因（ponytail）：每个 agent-type 的 emit 出口本就 bespoke（claude→emitToClaude / codex→codex.injectRoomNotice），Map 只统一存储不统一接线；实际需求是 claude+codex 两个，两绑定 diff 最小、Claude 处理器字节不变（daemon.ts 最敏感 + aikey 要 rebase）。第 3 个 agent-type 再加自己的绑定（YAGNI）。
   - boot 末尾（daemon.ts:2497）现有 Claude bridge 保留（emit→emitToClaude），**新增** Codex bridge：
     ```
     void startRoomBridge({
       cwd, log, agentType: "codex", capabilities: ["implement","execute"],
       emit: (text) => codex.injectRoomNotice(text),   // 见 1.3 排队
     }).then(h => { if (shuttingDown) h.stop(); else codexRoomBridge = h })
     ```
     （token 由 startRoomBridge 内 `readAuthToken(dbPath, "codex")` 自动取 auth-token-codex，见 1.4。）
   - 关停（daemon.ts:2402）：`roomBridge?.stop()` 后加 `codexRoomBridge?.stop()`。
   - **插入锚点**：声明处（:240）、boot 实例化处（:2497）、shutdown 处（:2402）。
