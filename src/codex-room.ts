...
export class CodexRoomInbox {
  ...
  flush(): void {
    if (this.stopped || Date.now() < this.retryAfter || this.active || !this.queue.length || !this.allowed() || !this.codex.canInject()) return;
  ...
  private finished = () => { this.inFlight = null; this.flightBatch = []; };
  ...
  private started = ({ requestId, turnId }: { requestId: number; turnId: string }) => {
    if (requestId === this.inFlight) { this.roomTurns.add(turnId); if (this.roomTurns.size > 500) this.roomTurns.delete(this.roomTurns.values().next().value!); }
  };
